...
from util.env import get_normalization_params
...
class PPO:
    def __init__(self, args, env_fn, policy, critic, save_path):
        ...
        self.policy = policy
        self.critic = critic
        worker_policy = self.policy.to('cpu')
        worker_critic = self.critic.to('cpu')
        self.workers = [Collect_Worker(self.policy, self.critic, self.env_fn,self.gamma, self.lam) for _ in range(self.num_worker)]
        # self.workers = [Collect_Worker.remote(worker_policy,worker_critic,self.env,self.gamma, self.lam) for _ in range(self.num_worker)]
