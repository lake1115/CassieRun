...
class Collect_Worker:
        ...
        with torch.no_grad():
            ...
            for traj in range(trajs):
                ...
                while not done and traj_len < max_traj_len:
                    ...
                    env_return = env.step(action.numpy())
                    if len(env_return == 4):
                        next_state, reward, done, _ = env_return
                    else:
                        next_state, reward, done, _, _ = env_return
    ...
    def meta_collect(self, max_traj_len, min_steps, task, anneal=1.0):
        torch.set_num_threads(1)
        with torch.no_grad():
            env = WrapEnv(self.env_fn)
            #env.speed = task
            ...
            while num_steps < min_steps:
                ...
                while not done and traj_len < max_traj_len:
                    ...
                    value = self.critic(torch.cat([state, self.policy.z],dim=1))

                    next_state, reward, done, _ = env.step(action.numpy())

                    ## transition for context training
                    transition = [state.numpy(), action.numpy(), reward, next_state]
                    
                    ##
                    memory.meta_store(state.numpy(), action.numpy(), reward, value.numpy(), self.policy.z.detach().numpy())
                    
                    self.policy.diffusion_context(transition)
                ...
                z_buffer.append(self.policy.z)    
                value = self.critic(torch.cat([state, self.policy.z],dim=1))
                memory.finish_path(last_val=(not done) * value.numpy())
            #policy_z = self.policy.z.detach().cpu().numpy()
            z_buffer = torch.cat(z_buffer,dim=0)
            return memory, z_buffer

    def meta_evaluate(self, max_traj_len, task, trajs=1):
        torch.set_num_threads(1)
        env = WrapEnv(self.env_fn)
        #env.speed = task
        env._goal_vel = task
        with torch.no_grad():
            ...
            for traj in range(trajs):
                ...
                while not done and traj_len < max_traj_len:
                    action = self.policy(state, transition)
                    next_state, reward, done, _ = env.step(action.numpy())
